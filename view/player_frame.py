...
class PlayerFrame(tk.Frame):
    def __init__(self, width, height, master=None):
        ...
        self.color_var = self.make_var("Color: red", 0, 0)
        self.wood_var = self.make_var("Num wood: 0", 1, 0)
        self.brick_var = self.make_var("Num brick: 0", 2, 0)
        self.sheep_var = self.make_var("Num sheep: 0", 3, 0)
        self.wheat_var = self.make_var("Num wheat: 0", 4, 0)
        self.stone_var = self.make_var("Num stone: 0", 5, 0)
        self.road_var = self.make_var("Num roads: 15", 0, 1)
        self.settle_var = self.make_var("Num settles: 5", 1, 1)
        self.city_var = self.make_var("Num cities: 4", 2, 1)
        self.army_size_var = self.make_var("Army size: 0", 3, 1)
        self.road_len_var = self.make_var("Road length: 0", 4, 1)
        self.vp_var = self.make_var("VP: 0", 5, 1)
